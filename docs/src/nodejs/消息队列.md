...
- **疑问2**
    ...
    答案：代码中我们声明的是路由是routingKey，但是它并没有创建hellocoQueue 消息队列，消息只会发送到交exchange交换机。
    运行代码后看队列截图可以证明这一点:
...
- **运行后的执行结果**
    ...
    ![](../../assets/image/queue_05.jpg)
- **说明1**

    这时候我改变代码中的队列名称为`hellocoQueueHaHa`,这时候去看Rabbitmq可视化界面中，队列模块，创建了这个队列

    ![](../../assets/image/queue_06.jpg)
    
    看到这里再次证明了消息队列优秀的`解耦特性`，`消费者和生产者模型`之间没有任何联系，再次创建这个`hellocoQueueHaHa`路由名称的生产者，`消费者`也会正常消费，并且会打印消息，大家可以实际操作试一下。
...
- **说明2**
    ...
    看到这里又再次证明了消息队列优秀的`解耦特性`，`消费者和生产者模型`之间没有任何联系，再次创建这个`hellocoQueueHaHa`路由名称的生产者，`消费者`也会正常消费，并且会打印消息，大家可以实际操作试一下。
### 如何释放掉消息队列
#### 可视化界面中直接删除掉消息队列
...
async function producer() {
    ...
    // 声明参数
    const exchangeName = 'fanout_co_exchange';
    const routingKey = '';
    const msg = 'hello co';
...
// 消费者消费时候的代码
await channel.consume(queueName, msg => {
    console.log('co：', msg.content.toString());
