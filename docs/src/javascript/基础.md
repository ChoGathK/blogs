...
> 所有 JavaScript 变量必须以唯一的名称的标识。 标识符可以是短名称（比如 x 和 y），或者更具描述性的名称（age、sum）。
...
在 JavaScript 中涉及到隐式转换的场景，大多是 JavaScript 引擎在解析运算符运算时产生的。而隐式转换主要是通过
