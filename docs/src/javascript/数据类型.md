...
> JavaScript 中，除了 null 和 undefined 一切皆对象，但是对象的值又可以被细分为 `对象/引用类型` 或者 `基础类型` 中的 **string**、**number**、**boolean**
...
1. null像在Java里一样，被当成一个对象。但是作者觉得表示"无"的值最好不是对象。
...
- **null** 不是一个对象，是一个空对象指针，尽管 typeof **null**  输出的是 Object。这是一个历史遗留问题，JS 的最初版本中使用的是 32 位系统，为了性能考虑使用低位存储变量的类型信息，000 开头代表是对象，**null** 表示为全零，所以 js 引擎将它错误的判断为 Object 。
- **null** 会在栈空间存储一个堆空间指针，**undefined** 会在栈空间存储一个空值，空指针比空值更占用内存
