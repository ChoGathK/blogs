...
> Nest.js 是 Node.js 的 web 后端框架，其设计思想借鉴了 Java Spring，旨在高效的构建一个松耦合、高内聚的应用

## 设计思想

### IoC (控制反转) & DI (依赖注入)

> `Nest.js` 将应用程序赋予了一个中心 ———— `Module` 容器，并将 `Providers`(依赖/提供者) 的创建过程提取到 `Module` 中，由 `Module` 向需要 `Providers` 的模块进行注入

#### 基本概念

* 容器 (模块): `Module` 将在运行时管理并组织各个模块间的依赖关系

* 提供者 (依赖): `Providers` 需要在 `Module` 中进行注册，并在目标类的 `constructor` 进行声明注入来完成依赖关系的关联

#### 概念描述

IoC 是一种方法论，一种工程化的思想。使用 IoC 意味着你将设计好的对象交给容器控制，而不是在对象内部直接控制，`Nest.js` 把创建和查找 `Providers` 的控制权交给了 `Module` ，由 `Module` 进行注入组合对象，所以对象与对象之间的耦合是松散的，有利于功能复用，更重要的是使得程序的整个体系结构变得非常灵活。

DI 指组件之间的依赖关系由 `Module` 在运行过程中规定。具象化一点，各个组件的依赖关系是由 `Module` 来注入的。
开发者依赖这个机制，只需要通过配置 `Module`，完成业务逻辑即可，不用关心具体的资源来自何方，由谁实现（依然是面向对象的思想）

#### 谁在控制谁

一般来讲，我们直接 `new` 一个对象，是我们运行的这个程序去主动的创建依赖对象；但是使用 IoC 模式开发时，会使用 `Module` 来负责这些对象的创建。这个时候是 `Module` 在控制对象。

#### 反转了什么

在传统的程序里面，我们在对象中主动控制去直接获取依赖对象，而现在，这个过程反了过来。由 `Module` 管理的 `Providers` 主动注入并寻找调用方。
...
* 因为使用反射技术来创建依赖对象，所以效率相对直接 `new` 要慢，而且创建依赖对象的过程将变得复杂
...
module.exports = {
  ...
  rules: {
     ...
     * 可能的错误
     *
     * 这些规则与 JavaScript/typeScript 代码中可能的语法错误或逻辑错误有关
     *
     */
    ...
    'no-await-in-loop': 'off', // @off 要求太严格了，有时需要在循环中写 await
    // 禁止与负零进行比较
    'no-compare-neg-zero': 'error',
    // 禁止在测试表达式中使用赋值语句，除非这个赋值语句被括号包起来了
    'no-cond-assign': ['error', 'except-parens'],
    // 禁止使用 console
    'no-console': 'off', // @off console 的使用很常见
    // 禁止将常量作为分支条件判断中的测试表达式，但允许作为循环条件判断中的测试表达式
    'no-constant-condition': ['error', { checkLoops: false }],
    // 禁止在正则表达式中出现 Ctrl 键的 ASCII 表示，即禁止使用 /\x1f/
    'no-control-regex': 'off', // @off 几乎不会遇到这种场景
    ...
    'class-methods-use-this': 'off', // @off 太严格了
    // 禁止函数的循环复杂度超过 20，https://en.wikipedia.org/wiki/Cyclomatic_complexity
    'complexity': ['error', { max: 20 }],
    // 禁止函数在不同分支返回不同类型的值
    'consistent-return': 'off',  // @off 太严格了
    // @fixable if 后面必须要有 {，除非是单行 if
    'curly': ['error', 'multi-line', 'consistent'],
    // switch 语句必须有 default
    'default-case': 'off', // @off 太严格了
    // @fixable 链式调用的时候，点号必须放在第二行开头处，禁止放在第一行结尾处
    'dot-location': ['error', 'property'],
    // @fixable 禁止出现 foo['bar']，必须写成 foo.bar
    'dot-notation': 'off',  // @off 当需要写一系列属性的时候，可以更统一
    // @fixable 必须使用 === 或 !==，禁止使用 == 或 !=，与 null 比较时除外
    'eqeqeq': ['error', 'always', { null: 'ignore' }],
    // for in 内部必须有 hasOwnProperty
    'guard-for-in': 'error',
    // 禁止使用 alert
    'no-alert': 'off', // @off alert 很常用
    // 禁止使用 caller 或 callee
    'no-caller': 'error',
    // switch 的 case 内有变量定义的时候，必须使用大括号将 case 内变成一个代码块
    'no-case-declarations': 'error',
    // 禁止在正则表达式中出现形似除法操作符的开头，如 let a = /=foo/
    'no-div-regex': 'off', // @off 有代码高亮的话，在阅读这种代码时，也完全不会产生歧义或理解上的困难
    // @fixable 禁止在 else 内使用 return，必须改为提前结束
    'no-else-return': 'off', // @off else 中使用 return 可以使代码结构更清晰
    ...
    'no-eq-null': 'off', // @off foo == null 用于判断 foo 不是 undefined 并且不是 null，比较常用，故允许此写法
    // 禁止使用 eval
    'no-eval': 'error',
    // 禁止修改原生对象
    ...
    'no-implicit-coercion': ['error', { allow: ['!!'] }], // 仅允许使用 “!!” 运算符
    // 禁止在全局作用域下定义变量或申明函数
