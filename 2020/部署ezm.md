...
Easy-Monitor 作者从 2014 年开始入坑 Node.js，也算是国内比较早开始从事全职 Node.js 开发的工程师。
早期被诸如有问题的三方库导致的内存泄漏以及正则回溯引起的 JS 主线程卡死等疑难杂症坑过，所以在写了 3 年业务开发后，从 2017 年开始基本上一直致力于 Node.js 性能监控工具链的研究与工作，Easy-Monitor 的各个版本可以说是每个研究阶段的总结。

## 一、背景

由于目前业务架构是基于 Docker + K8S 部署，作者推荐的主机部署方式无法满足需求，所以诞生了这篇指南，希望自己的踩过的一些坑可以为大家做一点微小的贡献 👓

## 二、写在部署前

### 1 部署依赖

* 代码仓库: 阿里云云效（免费） [CodeUp](https://codeup.aliyun.com)

* CI和自动部署: 阿里云云效（免费） [Flow](https://flow.aliyun.com)

* 镜像仓库: 阿里云容器镜像服务（免费） [容器镜像服务](https://cr.console.aliyun.com)

* K8S: 阿里云容器服务（付费） [容器服务](https://cs.console.aliyun.com)
...
![flow](https://s1.ax1x.com/2020/10/28/B1aRdP.png)
...
![monitor](https://s1.ax1x.com/2020/10/28/B1aWIf.png)
